...
export default function InvoicesPage() {
  ...
  useEffect(() => { load(); }, [status, search, dateFrom, dateTo, page]);
  ...
  return (
    <div className="p-6" dir="rtl">
      ...
      <div className="flex gap-2 mb-3 flex-wrap">
        {(["", "Draft", "Confirmed", "PartiallyPaid", "Paid", "Cancelled"] as const).map((s) => (
          <button
            key={s}
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        ...
        {(search || dateFrom || dateTo) && (
          <button onClick={() => { setSearch(""); setDateFrom(""); setDateTo(""); setPage(1); }} className="text-sm text-gray-500 border px-3 py-2 rounded-lg hover:bg-gray-50">إعادة تعيين</button>
        )}
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-200">
              {(data?.items ?? []).map((inv) => {
                ...
                return (
                  <tr key={inv.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-500">{inv.createdAt ? new Date(inv.createdAt).toLocaleDateString("ar-LY") : "—"}</td>
              ...
              {(data?.items.length ?? 0) === 0 && (
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-gray-400">لا توجد فواتير</td>
                </tr>
              )}
            </tbody>
          </table>
